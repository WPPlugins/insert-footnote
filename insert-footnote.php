...
Plugin URI: http://dev.coziplace.com/free-wordpress-plugins/insert-footnote 
...
function dk_create_menu() {
         ...
         add_options_page( 'Insert Footnote', 'Insert Footnote','manage_options', 'wimp', 'dk_menu_page');
}
...
function dk_setting_input(){
    ...
    $home='';
     ...
     echo "<br>Hide from : <input type='checkbox' id='page' value='checked' name='dk_options[page]' ".$page."> Page <input type='checkbox' value='checked' name='dk_options[home]' ".$home."> Home <input type='checkbox' value='checked' name='dk_options[tax]' ".$tax."> Tags&Categories" ; 
...
function dk_insert_footer($content){
    ...
    $options = get_option('dk_options');
     ...
     $home = $options['home'];
     $tax = $options['tax'];
   if (is_page() && $page=='checked' ) $text_string = '';
   if (is_home() && $home=='checked' ) $text_string = '';
   if ((is_tag() || is_category()) && $tax=='checked' ) $text_string = '';
